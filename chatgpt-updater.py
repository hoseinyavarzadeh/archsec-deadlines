...
os.environ["OPENAI_API_KEY"] = "" # update this with your key
os.environ["SERPER_API_KEY"] = "" # update this with your key
os.environ["COLLECT_TRAINING_DATA"] = "0"
...
Consider only information from authoritative sources, such as the conference's website or the conference organizer or the publisher of the conference proceedings, not from third-party websites. If you find the information on a third-party website, but cannot readily find it from official sources, then eventually return no update.
...
Subject areas in `sub` are `CA` (computer architecture), `BC` (blockchain), `CR` (cryptography), `DS` (distributed systems), `SEC` (security), and `EC` (economics/incentives).
...
class Conference(BaseModel):
    ...
    sub: list[Literal["CA", "BC", "CR", "DS", "SEC", "EC"]]
    inactive: bool
